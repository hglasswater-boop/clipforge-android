...
class AutoTrimProgressTest {
    ...
    fun etaIsHiddenUntilEnoughRealWorkHasCompleted() {
        ...
        now = 1_000L
        val early = tracker.update(AutoTrimPhase.START_AUDIO, 0.1)
    ...
    fun etaUsesElapsedTimeAndActualPhaseProgress() {
        ...
        now = 2_000L
        val progress = tracker.update(AutoTrimPhase.START_AUDIO, 0.5)
    ...
    fun overallProgressNeverMovesBackwards() {
        ...
        now = 3_000L
        val staleUpdate = tracker.update(AutoTrimPhase.START_AUDIO, 0.2)
